    ...
    def eval_episodes(target_rew, env_name):
        def fn(model):
            ...
            for i in range(num_eval_episodes):
                ...
                with torch.no_grad():
                    if model_type == 'dt':
                        ret, length = evaluate_episode_rtg(env, state_dim, act_dim, model, max_ep_len=max_ep_len, scale=scale, target_return=target_rew/scale, 
                                                           mode=mode, state_mean=state_mean, state_std=state_std, device=device,)
                    else:
                        ret, length = evaluate_episode(env, state_dim, act_dim, model, max_ep_len=max_ep_len, target_return=target_rew/scale, mode=mode, 
            ...
            return {
                ...
                # 'target_{}_score  _mean'.format(target_rew): np.mean(scores),
            }
        return fn
    

     # Define the parameter grid
    # max_len_aug_values = [2000, 1000]
    max_len_aug_values = [4000]
    no_search_action_values = [7, 5]
    step_action_search_values = [0.01]
    sampling_aug_ratio_values = [1, 2]   

    for max_len_aug in max_len_aug_values:
        for no_search_action in no_search_action_values:
            for step_action_search in step_action_search_values:
                for sampling_aug_ratio in sampling_aug_ratio_values:
                    if max_len_aug == 2000 and no_search_action == 7 and step_action_search == 0.01 and sampling_aug_ratio == 1:
                        continue
                    # Update args with current parameter values
                    args.max_len_aug = max_len_aug
                    args.no_search_action = no_search_action
                    args.step_action_search = step_action_search
                    args.sampling_aug_ratio = sampling_aug_ratio
                    ...
                    if args.use_data_augmentation and (args.train_augment_model is False): 
                        if args.use_less_data:
                            ...
                            if env_name == "hopper" or env_name == "halfcheetah":
                                save_dir = "saved_model/{}_{}_{}_modified/".format(env_name, args.dataset, args)
                            else:
                    ...
                    if model_type == 'dt':
                        ...
                        trainer = ActTrainer(model=model, optimizer=optimizer, batch_size=batch_size, get_batch=get_batch, scheduler=scheduler, 
                                            ...
                                            args=args)
                    if log_to_wandb:
                        name = '{}_{}_{}_{}_{}'.format(exp_prefix, max_len_aug, no_search_action, step_action_search, sampling_aug_ratio)  

                        wandb.init(
                            name=name,
                            group=group_name,
                            project='decision-transformer_v_hcm_gs',
                            config= vars(args)
                        )
...
if __name__ == '__main__':
    ...
    parser.add_argument('--under_loading', type=str2bool, default="True")
